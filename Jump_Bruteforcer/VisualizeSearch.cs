...
namespace Jump_Bruteforcer
{
    ...
    public static class VisualizeSearch
    {
        ...
        /*public static void CountStates(SimplePriorityQueue<PlayerNode, uint> openSet, HashSet<PlayerNode> closedSet)
        {
            ...
            MaxStates(openSet, false);
            MaxStates(closedSet, true);

        }

        private static void MaxStates(IEnumerable<PlayerNode> set, bool closed)
        {
        ...
        public static WriteableBitmap HeatMap()
        {
            ...
            int maxStatesPerPx = Search.maxStates;
            for (int X = 0; X < Map.WIDTH; X++)
            {
                for (int Y = 0; Y < Map.HEIGHT; Y++)
                {
                    if (!Search.statesPerPx.ContainsKey((X, Y)))
                    {
                        continue;
                    }

                    (int Open, int Closed) = Search.statesPerPx[(X, Y)];
                    ...
                    double Brightness = (double)Closed / Math.Max(Open, Closed);
        ...
        public static void LiveHeatmap()
        {
            void UpdateHeatmap()
            {
                WriteableBitmap bitmap = HeatMap();
                bitmap.Freeze();

                Dispatcher dispatcher = MainWindow.ActiveWindow.Dispatcher;

                dispatcher.Invoke(() => MainWindow.ActiveWindow.ImageHeatMap.Source = bitmap);
            }

            while (Search.searchRunning)
            {
                UpdateHeatmap();
